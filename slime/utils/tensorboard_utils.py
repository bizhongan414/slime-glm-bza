...
class _TensorboardAdapter(metaclass=SingletonMeta):
    ...
    def __init__(self, args):
        ...
        os_env_tb_dir = os.environ.get("TENSORBOARD_DIR", None)
        if tb_project_name is not None or os_env_tb_dir is not None or tensorboard_dir is not None:
            if os_env_tb_dir is not None:
                self._initialize(os_env_tb_dir)
            elif tensorboard_dir is not None:
                self._initialize(tensorboard_dir)
            elif tb_project_name is not None and tb_experiment_name is None:
    ...
    def _initialize(self, tensorboard_dir):
