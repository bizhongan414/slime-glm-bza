...
DEFAULT_CODE_SYSTEM_PROMPT = (
    ...
    "When computation or code execution is needed, call the `code_interpreter` tool. "
    "\n\nCRITICAL INSTRUCTIONS FOR CODE EXECUTION:\n"
    "1. The tool executes Python code and returns ONLY the standard output (stdout).\n"
    "2. You MUST print the final result to stdout using the `print()` function on the last line of your code.\n"
    "3. If you do not print the result, you will receive an empty response and fail the task.\n"
    "\nProvide tool calls strictly using the model's required tool-calling format."
)


def strip_reasoning_from_response_text(response_text: str) -> str:
    """Remove explicit <think>...</think> content from assistant text before storing it in history."""
    if not response_text:
        return response_text
    if "</think>" in response_text:
        return response_text.split("</think>", 1)[1].lstrip()
    return response_text
...
class AgentLoop:
    ...
    def __init__(self, 
        ...
        if evaluation:
            self.max_turns = getattr(args, "eval_rollout_max_turn", getattr(args, "rollout_max_turn", 5))
            self.max_response_length = getattr(
                args,
                "eval_max_response_len",
                getattr(args, "rollout_max_response_len", 4096),
            )
        else:
    ...
    async def _call_tool(
        ...
        try:
            ...
            return result, tool_reward, tool_meta
            
        except Exception as e:
            logger.warning(f"Error when executing tool '{tool_call.name}': {e}")
            return f"Error when executing tool: {e}", 0.0, {"sandbox_runtime_error": 1.0, "sandbox_success": 0.0}
    ...
    async def _handle_generating_state(self, agent_data: AgentData) -> AgentState:
        ...
        logger.info(
            "Generation turn=%s assistant_turn=%s response_chars=%s completion_tokens=%s",
            agent_data.turn_idx,
            agent_data.assistant_turns + 1,
            len(response_text),
            meta_info.get("completion_token_num"),
        )
        logger.debug("Assistant raw response: %s", response_text)
        
        # Append assistant response to message history. For thinking-model experiments we
        # can optionally drop explicit reasoning text from history while still preserving
        # tool calls / visible content for the next turn.
        history_content = response_text
        if getattr(self.args, "code_strip_reasoning_from_history", False):
            history_content = strip_reasoning_from_response_text(response_text)
        agent_data.messages.append({"role": "assistant", "content": history_content})
        ...
        if tool_calls:
            logger.info("Detected %s tool call(s) on turn=%s", len(tool_calls), agent_data.turn_idx)
            for idx, tool_call in enumerate(tool_calls):
                try:
                    ...
                    logger.debug("Tool call[%s]: %s", idx, tool_call)
        if tool_calls:
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        async def call_tool_with_semaphore(idx: int, tool_call: FunctionCall) -> tuple[int, str, Optional[float], dict[str, Any]]:
            """Execute tool with semaphore-controlled concurrency, return with index for ordering."""
            async with semaphore:
                # breakpoint()
                result_text, tool_reward, tool_meta = await self._call_tool(tool_call, agent_data)
                return idx, result_text, tool_reward, tool_meta
        ...
        add_messages = []
        sample_metadata = agent_data.sample.metadata if isinstance(agent_data.sample.metadata, dict) else {}
        sample_metadata.setdefault("sandbox_tool_calls", 0.0)
        sample_metadata.setdefault("sandbox_finished", 0.0)
        sample_metadata.setdefault("sandbox_success", 0.0)
        sample_metadata.setdefault("sandbox_compile_error", 0.0)
        sample_metadata.setdefault("sandbox_runtime_error", 0.0)
        sample_metadata.setdefault("sandbox_no_output", 0.0)
        for idx, result_text, tool_reward, tool_meta in indexed_responses:
            # Format observation message (use 'tool' role for tool responses)
            logger.info(
                "Tool result[%s] chars=%s tool_reward=%s",
                idx,
                len(result_text),
                tool_reward,
            )
            logger.debug("Tool result[%s] content: %s", idx, result_text)
            observation_message = {
                "role": "tool",
            ...
            sample_metadata["sandbox_tool_calls"] += 1.0
            for key in [
                "sandbox_finished",
                "sandbox_success",
                ...
                sample_metadata[key] += float(tool_meta.get(key, 0.0))
        ...
        agent_data.current_tool_calls = []
        agent_data.sample.metadata = sample_metadata
